...
const transporter = nodemailer.createTransport(sendgridTransport({
    auth: {
        api_key: 'send grid api key from setting'
    }
}))
...
const stripe = require('stripe')('secret key stripe');
...
exports.getCheckout = (req, res, next) => {
    let products;
    let total = 0;
    req.user.getCart().then(productss => {
        products = productss;
        products.forEach(p => {
            total += p.quantity * p.price;
            console.log(p.price);
        ...
        return stripe.checkout.sessions.create({
            payment_method_types: ['card'],
            //which items will be checkout out
            line_items: products.map(p => {
                return {
                    price_data: {
            ...
            mode: "payment",
            //stripe urls that will be redirected once it successfully makes it
            success_url: 'http://localhost:3000/checkout/success',
            cancel_url: 'http://localhost:3000/checkout/cancel'
            /*success_url: req.protocol + '://' + req.get('host') + '/checkout/success',  //req.protocol = http and adding :// will become http://
...
module.exports = (req, res, next) =>{
    ...
    //otherwise we calll next
    // we want the other request to continue
    //to which ever route the request is going to be
    next();
}
...
const deleteProduct = (btn) =>{
    ...
    //if no http it will run to current host
...
//when we send the request from javascript then router allows you to send delete routes too
    ...
    <main>
        <% if (products.length > 0) { %>
            ...
            <div class="centered">
                ...
                <!-- we need get type of request not post request hence a href will work-->
        ...
        <%- include('../includes/navigation.ejs') %>
            <main>
                ...
                <div class="centered">
                    ...
                    <script>
                        var stripe = Stripe('private key from stripe');
