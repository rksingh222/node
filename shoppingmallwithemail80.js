...
exports.getAddProduct = (req, res, next) => {
    res.render('admin/add-product', { pageTitle: 'Add Product', path: '/admin/add-product', editing: false ,isAuthenticated: req.session.isLoggedIn});
}
...
exports.postSignup = (req, res, next) => {
    ...
    User.findByEmail(email).then(userDoc => {
        ...
        return bcrypt.hash(password, 12).then(hashedPassword => {
            const user = new User(email, hashedPassword, { items: [] }, null);
            return user.save();
        }).then(result => {
            res.redirect('/login');
...
exports.getProducts = (req, res, next) => {
    Product.fetch().then(products => {
        res.render('shop/product-list', { prods: products, docTitle: 'All Products', path: '/products' ,isAuthenticated: req.session.isLoggedIn});
    }).catch(err => {
        console.log(err);
    });
}
...
//create data folder in main directory
